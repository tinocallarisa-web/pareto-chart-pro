...
## High Performance & Big Data Streaming
- **Window Data Reduction:** Configured with `"window": { "count": 30000 }` to support datasets from 30,000 up to 150,000+ rows via `fetchMoreData(true)`.
- **Segment Event Safety:** Every segment fetch path emits `renderingFinished(options)` to prevent host freezes.
